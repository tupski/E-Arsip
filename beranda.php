...
if(!isset($_SESSION['admin'])){
    $_SESSION['err'] = '<div class="alert alert-error">Anda harus login terlebih dahulu!</div>';
    header("Location: ./");
    die();
} else {
    ...
    // Total Berita Acara
    $query = mysqli_query($config, "SELECT COUNT(*) as total FROM tbl_berita_acara" . ($_SESSION['admin'] > 2 ? " WHERE id_user='$id_user'" : ""));
    $data = mysqli_fetch_array($query);
    $total_berita = $data['total'];

    // Total Kendaraan
    $query = mysqli_query($config, "SELECT COUNT(*) as total FROM tbl_kendaraan" . ($_SESSION['admin'] > 2 ? " WHERE id_user='$id_user'" : ""));
    $data = mysqli_fetch_array($query);
    $total_kendaraan = $data['total'];

    // Total User (Admin Only)
    if($_SESSION['admin'] == 1 || $_SESSION['admin'] == 2){
        $query = mysqli_query($config, "SELECT COUNT(*) as total FROM tbl_user");
        $data = mysqli_fetch_array($query);
    ...
    <div class="col s12 m6 l4">
        <div class="card hoverable">
            ...
            <div class="card-action teal darken-1">
                <a href="?page=kendaraan" class="white-text" style="display: inline-flex; align-items: center;">
                    <i class="material-icons" style="margin-right: 4px;">directions_car</i> Kelola Data
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="col s12 m6 l4">
        <div class="card hoverable">
            <div class="card-content orange white-text">
                ...
                <div class="progress white">
                    <div class="determinate orange lighten-2" style="width: 30%"></div>
                </div>
            </div>
            <div class="card-action orange darken-1">
                <a href="?page=usr" class="white-text" style="display: inline-flex; align-items: center;">
                    <i class="material-icons" style="margin-right: 4px;">people</i> Kelola User
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="card hoverable">
            <div class="card-content">
                ...
                <table class="striped responsive-table">
                    ...
                    <tbody>
                        <?php
                        $query = mysqli_query($config, "SELECT * FROM tbl_berita_acara" . ($_SESSION['admin'] > 2 ? " WHERE id_user='$id_user'" : "") . " ORDER BY tgl_pembuatan DESC LIMIT 5");
                        if(mysqli_num_rows($query) > 0){
                            ...
                            while($row = mysqli_fetch_array($query)){
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="?page=detail_berita&id='.$row['id_berita_acara'].'" class="btn-flat waves-effect blue-text">
